...
export async function pauseJob(id: number): Promise<void> {
  const res = await authFetch(`${BASE}/jobs/${id}/pause`, { method: 'PATCH' });
...
export async function retryJob(id: number): Promise<Job> {
  const res = await authFetch(`${BASE}/jobs/${id}/retry`, { method: 'POST' });
...
export async function deleteJobsBulk(status: 'DONE' | 'FAILED'): Promise<{ deleted: number }> {
  const res = await authFetch(`${BASE}/jobs/bulk`, {
    method: 'DELETE',
    headers: { 'Content-Type': 'application/json' },
